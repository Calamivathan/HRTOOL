...
def unpak64(selected_file):
    ...
    try:
        ...
        subprocess.run(["qemu-i386",f"{quickbms}/quickbms_4gb_files", "-e", "-w", bms_path, f"{original_pak}/{os.path.basename(selected_file)}", unpacked_pak], check=True)
...
def repak64(selected_file):
    ...
    try:
        ...
        subprocess.run(["qemu-i386",f"{quickbms}/quickbms_4gb_files", "-w", "-r", bms_path, selected_file, Edited_DEASS_DAT], check=True)
        deletion_thread.join()
    except subprocess.CalledProcessError as e:
        pass  # Suppress the error
